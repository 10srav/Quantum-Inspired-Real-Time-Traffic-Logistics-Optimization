...
class TestQUBOOptimizer:
    ...
    def test_optimize_performance_5_locations(self, optimizer):
        """Test optimization completes in <5s for 5 locations."""
        n = 5
        np.random.seed(42)
        dist_matrix = np.random.rand(n, n) * 1000
        dist_matrix = (dist_matrix + dist_matrix.T) / 2
        ...
        assert total_time < 5.0, f"Optimization took {total_time:.2f}s, expected <5s"
        assert len(sequence) == n
